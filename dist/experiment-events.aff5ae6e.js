let e="https://app.ticketmaster.com/discovery/v2/",t="JIZUA78ORWWvAkFITEgp9n4NpYKrXysZ";async function a(a){let n=await fetch(e+`events.json?apikey=${t}&size=20&page=${a}`);return await n.json()}async function n(a){let n=await fetch(e+`events/${a}.json?apikey=${t}`);return await n.json()}async function s(a,n){let s=await fetch(e+`events.json?apikey=${t}&size=20&page=${a}&keyword=${n}`);return await s.json()}async function l(a,n){let s=await fetch(e+`events.json?apikey=${t}&size=20&page=${a}&countryCode=${n}`);return await s.json()}let d=document.querySelector(".cards_list");function c(e){d.innerHTML="",e.forEach(e=>{d.innerHTML+=`
    ...
    `,document.body.appendChild(t),t.querySelector(".modal_close").addEventListener("click",()=>{t.remove()}),t.querySelector(".modal_favorite").addEventListener("click",()=>{let t=JSON.parse(localStorage.getItem("favorites"))||[];t.push(e.id),localStorage.setItem("favorites",JSON.stringify(t))}),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.addEventListener("keydown",e=>{"Escape"===e.key&&t.remove()})})}),document.querySelector(".cards_pag").addEventListener("click",e=>{let t=e.target.closest(".pag_button");t&&(u=Number(t.dataset.page),m?_():p?g():v())});let y=document.querySelector(".nav_select");async function g(){let e=await l(u,p);c(e._embedded.events),i(e.page.totalPages,u)}y.addEventListener("change",()=>{p=y.value,m=null,u=0,document.querySelector(".cards_info").textContent=`Resulst from the country of ${y.querySelector(`option[value=${y.value}]`).textContent}:`,g()});
//# sourceMappingURL=experiment-events.aff5ae6e.js.map
